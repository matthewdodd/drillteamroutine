...
function initSim() {
    ...
    if (!document.getElementById('structural-indicator')) {
        const indicator = document.createElement('div');
        indicator.id = 'structural-indicator';
        document.getElementById('grid-floor').appendChild(indicator);
    }
...
function detectMode() {
    ...
    return (diff < 5 || Math.max(diff, 180-diff) < 5 || Math.abs(diff - 180) < 5) ? "LINE" : "COLUMN";
}
...
async function runSequence() {
    ...
    for (let it of items) {
        ...
        squad.forEach(s => { 
            s.turning = false; 
            s.stepsDone = 0; 
            s.localFacing = visualFacing; // Maintain raw degree value during the move
        });
        ...
        // Critical: Update structural front if we wheeled in line
        if (m.includes("Wheel") && detectMode() === "LINE") {
            structuralFront = currentPivotGate.targetFacing;
        }

        // Only normalize visualFacing AFTER the entire move logic is calculated
        visualFacing = currentPivotGate ? currentPivotGate.targetFacing : visualFacing;
...
function execute(type, totalPaces) {
    return new Promise(resolve => {
        ...
        if (type.includes("Turn")) {
            ...
            const wheelPaces = mode === "LINE" ? Math.max(8, (maxFIdx + 1) * 2) : 4;
            ...
            squad.forEach(s => {
                if (mode === "LINE") {
                    s.turning = true; // Everyone wheels together in Line
                } else if (s.rIdx !== -1) {
                    const dist = Math.sqrt(Math.pow(s.x - currentPivotGate.x, 2) + Math.pow(s.y - currentPivotGate.y, 2));
                    if (!s.turning && s.stepsDone === 0 && dist < 25) {
                        squad.filter(m => m.fIdx === s.fIdx).forEach(m => m.turning = true);
                    }
                }
                // Commander follows if squad is turning
                if (s.rIdx === -1 && squad.some(m => m.turning && m.rIdx !== -1)) s.turning = true;
            });
            ...
            if (statusDisp) statusDisp.innerText = `State: ${isOpened ? "Open" : "Closed"} Order`;
        }
...
function updateVisuals() {
    ...
    squad.forEach(m => {
        ...
        if(currentPivotGate && Math.abs(m.x - currentPivotGate.x) < 5 && Math.abs(m.y - currentPivotGate.y) < 5) {
            m.el.classList.add('is-pivot');
        } else {
            m.el.classList.remove('is-pivot');
        }
    });

    // Update the Structural Front Indicator
    const indicator = document.getElementById('structural-indicator');
    if (indicator && squad.length > 0) {
        const drillMembers = squad.filter(s => s.rIdx !== -1);
        const avgX = drillMembers.reduce((sum, s) => sum + s.x, 0) / drillMembers.length;
        const avgY = drillMembers.reduce((sum, s) => sum + s.y, 0) / drillMembers.length;
        
        indicator.style.left = (avgX + 18) + 'px'; 
        indicator.style.top = (avgY - 40) + 'px';
        indicator.style.transform = `rotate(${structuralFront}deg)`;
    }
}
